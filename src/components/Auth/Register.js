...
const Register = ({ login }) => {
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			dispatch(registerUser(values, handleRedirect));
		}
	});
	return (
		<Auth>
			<Auth.Wrapper className="container">
				...
				<Auth.Form onSubmit={formik.handleSubmit}>
					<Input.Wrapper>
						...
						{errors && errors.email ? <small className="form-errors">{errors.email}</small> : null}
					</Input.Wrapper>
					<Input.Wrapper>
						<Input
							type="text"
						...
						{errors && errors.username ? <small className="form-errors">{errors.username}</small> : null}
					</Input.Wrapper>
					<Input.Wrapper>
						<Input
							type="password"
